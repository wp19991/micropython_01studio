...
static void xpt2046_corr(xpt2046_obj_t *self, int16_t * x, int16_t * y)
{
    ...
    (*x) = (uint32_t)((uint32_t)(*x) * lcddev.width) /
           (self->x_max - self->x_min);

    (*y) = (uint32_t)((uint32_t)(*y) * lcddev.height) /
	...
	while(1)
	{
		...
		if(xpt2046_read(&x, &y)){
			...
			switch(p_cont)
			{
				...
				break;
				case 2:
					lcd_Fill(lcddev.width-25,0,lcddev.width-5,25,WHITE);
					mp_hal_delay_ms(100);
				...
				break;
				case 3:
					lcd_Fill(lcddev.width-25, lcddev.height-25,lcddev.width-5,lcddev.height-5,WHITE);
					mp_hal_delay_ms(100);
				...
				case 4:
					...
					lcd_Fill(5, lcddev.height-25,25,lcddev.height-5,WHITE);
...
STATIC mp_obj_t xpt2046_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *all_args) {
	...
	self->spi = NULL;
	self->mhz = 5;
	self->spihost = g_ILI9341->spihost;
	self->cs 	= XPT_PIN_CS;
